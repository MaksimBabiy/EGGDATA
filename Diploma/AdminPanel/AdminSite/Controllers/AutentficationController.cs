namespace Server.Controllers
{
    ...
    public class AutentficationController : Controller
    {
        ...
        public async Task<IActionResult> Exchange(OpenIdConnectRequest requestt)
        {
            Debug.Assert(
                requestt.IsTokenRequest(),
            ...
            if (requestt.IsPasswordGrantType())
            {
                var user = await this.userManager.FindByNameAsync(requestt.Username);
                if (user == null)
                {
                ...
                var result = await this.signInManager.CheckPasswordSignInAsync(user, requestt.Password, lockoutOnFailure: true);
                if (!result.Succeeded)
                {
                    return this.BadRequest(new OpenIdConnectResponse
                    {
                ...
                var ticket = await this.CreateTicketAsync(requestt, user);

                return this.SignIn(ticket.Principal, ticket.Properties, ticket.AuthenticationScheme);
            }
            else if (requestt.IsAuthorizationCodeGrantType() || requestt.IsRefreshTokenGrantType())
            {
                ...
                var ticket = await this.CreateTicketAsync(requestt, user, info.Properties);
